<?php
namespace Fab\RedirectMessages\Override\Core\Mail;
 ...
 */
use Fab\RedirectMessages\Configuration\ConfigurationUtility;
...
class MailMessage extends \TYPO3\CMS\Core\Mail\MailMessage {
	...
	public function send() {
		$redirectTo = $this->getRedirectToForCurrentContext();
		...
		if (!empty($redirectTo)) {
			...
			$subject = strtoupper((string)GeneralUtility::getApplicationContext()) . ': ' . $this->getSubject();
	 ...
	 * @return array
	 */
	protected function getRedirectToForCurrentContext() {
	...
	/**
	 * @return \Fab\RedirectMessages\Validator\EmailValidator
	 */
	public function getEmailValidator() {
		return GeneralUtility::makeInstance('Fab\RedirectMessages\Validator\EmailValidator');
	}
}
